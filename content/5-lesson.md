...
The dataset  `QLDDriverReviverStations.csv` details rest-stop locations and facilities for drivers in Queensland. We are going to create an interactive map of this data, using geo.json.  However the geo.json tool cannot parse the data yet as it not tidy.
...
- Change  `(blank)` to  `No`  and close facet
